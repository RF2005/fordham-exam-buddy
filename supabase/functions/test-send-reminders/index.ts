...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log("Starting manual exam reminder test...");
    ...
    for (const pref of preferences as NotificationPreference[]) {
      ...
      for (const exam of exams as Exam[]) {
        ...
        console.log(`Exam ${exam.title}: ${daysUntilExam} days away. Reminder days: ${pref.reminder_days.join(', ')}`);
    ...
    console.error("Error in test-send-reminders function:", error);
    return new Response(
